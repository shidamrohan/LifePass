...
class _EmergencyQrScreenState extends State<EmergencyQrScreen> {
  ...
  Future<String> _load() async { final r = await apiService.get<Map<String, dynamic>>('/qr/generate', fromJson: (j) => j as Map<String, dynamic>); if (!r.success) throw Exception(r.error); return r.data?['encrypted_token'] as String? ?? ''; }
